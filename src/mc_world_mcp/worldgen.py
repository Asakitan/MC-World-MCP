...
_RESOURCE_CACHE: dict[tuple[str, str, tuple[tuple[str, int, int], ...]], tuple[float, list[dict[str, Any]]]] = {}
_INTERFACE_CACHE: dict[tuple[str, str, bool, int, int, tuple[tuple[str, int, int], ...], tuple[tuple[str, int, int], ...], tuple[tuple[str, int, int], ...]], tuple[float, dict[str, Any]]] = {}
...
def list_generation_interfaces(
    ...
    datapack_signature = _tree_signature(config.world / "datapacks")
    mod_signature = _archive_signature(config.root / "mods")
    plugin_signature = _archive_signature(config.root / "plugins")
    cache_key = (
        str(config.root),
        config.world_name,
        bool(include_archive_resources),
        int(max_archives),
        int(max_resource_samples),
        datapack_signature,
        mod_signature,
        plugin_signature,
    )
    cached = _cache_get(_INTERFACE_CACHE, cache_key)
    ...
    _cache_set(_INTERFACE_CACHE, cache_key, result)
    return result
...
def list_worldgen_resources(config: ServerConfig, namespace: str = "", type: str = "") -> list[dict[str, Any]]:
    ...
    resources = _all_worldgen_resources(config)
    ...
    return sorted(filtered, key=lambda item: (item["type"], item["id"], item["pack"]))


def _all_worldgen_resources(config: ServerConfig) -> list[dict[str, Any]]:
    signature = _tree_signature(config.world / "datapacks")
    cache_key = (str(config.root), config.world_name, signature)
    ...
    resources = sorted(resources, key=lambda item: (item["type"], item["id"], item["pack"]))
    _cache_set(_RESOURCE_CACHE, cache_key, resources)
    return resources
...
def _tree_signature(root: Path) -> tuple[tuple[str, int, int], ...]:
    ...
    for path in root.rglob("*"):
        ...
        try:
            ...
            continue
        result.append((path.relative_to(root).as_posix(), stat.st_mtime_ns, stat.st_size))
    return tuple(sorted(result))


def _archive_signature(root: Path) -> tuple[tuple[str, int, int], ...]:
    if not root.exists():
        return ()
    result: list[tuple[str, int, int]] = []
